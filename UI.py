...
class Identifier(QMainWindow):
    ...
    def initUI(self):
        ...
        browse = QPushButton("选择患者图片", self)
        browse.move(400, 555)
        ...
        self.setWindowTitle("Strabismus Recognizer")
    ...
    def openImage(self):
        ...
        try:
            filePath = fName[0]
            Recog_result, score = label(filePath)
            self.line.setText(Recog_result + " " + str(round(score, 2)) + "%")
            self.label.setPixmap(QPixmap(filePath).scaled(self.width(), self.height(), Qt.KeepAspectRatio))
        except:
            self.label.setPixmap(QPixmap('images/NoImage.jpg').scaled(self.width(), self.height(), Qt.KeepAspectRatio))
            self.line.setText("没有图片被选择！")
